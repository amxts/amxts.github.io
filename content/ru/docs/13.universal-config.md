...
Статус: проверено на сервере 2026-09-24 - TypeScript-плагин заменил
universal_config.amxx; menu_core и nhnse читают через него свои конфиги без
изменений. Без сервера: `tests/universal-config.test.ts` (файлы, записанные
оригиналом, возвращаются байт в байт).
...
(menu-core читает из неё `nhnse/menu.ini`, nhnse-core - `core.ini`), а файл,
...
Плагин `as/universal-config.ts` отдаёт Pawn-плагинам 28 нативов
...
см. natives.md), так что
собранные `.amxx` (menu_core и другие) работают с ним без изменений. Он
