...
У пакета-модуля — `@amxts/menu-core`, `@amxts/config-core`, любого
...
| `T[]`, `T \| null` | как есть |
| `Player` | по id |
| объект, который модуль отдаёт и принимает обратно (`Config`, `Section`), или отдаёт со своими методами (`Menu`) | объект владельца: поля читаются и пишутся у него, методы выполняются там же |
...
возвращает, другая принимает (`load()` возвращает `Config`,
`section(config, ...)` его принимает); классу со своими методами достаточно,
чтобы его отдавали (`create()` возвращает `Menu`, а `menu.addItem(...)`
выполняется у владельца). Такой объект — один и тот же во всех плагинах:
`find("X")` дважды — один и тот же `Menu`, `menu.title = "..."` меняет меню
владельца, а функция, переданная его методу, — `onSelect`, — вызывается
обратно там, где написана. Его приватные поля остаются у владельца, а поле
`readonly` из другого плагина тоже не записать.
