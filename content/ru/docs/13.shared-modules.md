...
У модуля (`~/modules/<имя>`), для которого есть плагин с тем же именем, —
`~/modules/menu-core` и плагин menu-core, `~/modules/universal-config` и
плагин universal-config — **один экземпляр на сервер**, экземпляр этого
плагина. Все плагины пишут один и тот же импорт:
...
Плагина, чей файл называется как модуль: `<имя>.ts` в папке плагинов владеет
`~/modules/<имя>`. На сервере, где плагин собирается из папки плагинов,
