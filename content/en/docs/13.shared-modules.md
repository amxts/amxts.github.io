...
A module (`~/modules/<name>`) that has a plugin of the same name —
`~/modules/menu-core` and the menu-core plugin, `~/modules/universal-config`
and the universal-config plugin — has **one instance on the server**, that
plugin's. Every plugin writes the same import:
...
The plugin whose file has the module's name: `<name>.ts` in the plugins
folder owns `~/modules/<name>`. On a server, where a plugin is compiled from
the plugins folder, a `<name>.aot` there counts too. The owner has to be in
