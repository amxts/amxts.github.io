...
menu-core and config-core are modules.
...
It writes a working module — a greeter to replace with your own — with its
playground project and a test that passes, as Nuxt's module starter does.
...
- `testing` — the module's test kit, for a module that needs more from the
  fake server than it has ([testing a module](/docs/testing#a-module-s-test-kit)).
...
`@amxts/core/kit` is what a module's code needs besides the facade, as
`@nuxt/kit` is for a Nuxt module. A plugin never needs it.
...
A module that replaces a Pawn plugin — as config-core replaces
...
```ts twoslash
// @filename: node_modules/@you/greeter/index.ts
import { Player } from "@amxts/core";

export function greet(player: Player) {
	// the installed module: its src/index.ts, above
}

// @filename: plugins/welcome.ts
...
A module is tested on the fake server ([testing](/docs/testing)), from its own
folder, with `npm test`. `setup()` loads a project there as the build would
lay it out on a server: its modules in load order, then its plugins.
...
kit](/docs/testing#a-module-s-test-kit).
