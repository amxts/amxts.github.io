...
WHEN NOT TO USE: If you already know the specific page path (e.g., "/docs/plugin"), use get-page directly instead.
...
- url: Full URL for reference`,
  ...
  handler: async () => {
    ...
    try {
      const pages = await queryCollection(event, 'docs_en')
